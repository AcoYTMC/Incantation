...
public class IncaWingsFeatureRenderer extends FeatureRenderer<PlayerEntityRenderState, PlayerEntityModel> {
    ...
    public IncaWingsFeatureRenderer(FeatureRendererContext<PlayerEntityRenderState, PlayerEntityModel> context, LoadedEntityModels modelLoader) {
        super(context);
        this.model = new IncaWingsModel(modelLoader.getModelPart(IncaModelLayers.INCA_WINGS), RenderLayer::getEntityCutoutNoCull);
    }

    public static boolean shouldRender(PlayerEntity player) {
        return IncaUtils.isWingedIncaling(player);
    }
